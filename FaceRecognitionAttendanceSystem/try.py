...
class face_recognition:
    ...
    def mark_attendance(self, recognised_student_roll, recognised_student_name, recognised_student_dep):
        with open("attendance.csv", "r+", newline="\n") as attendence:
            myDataList = attendence.readlines()
            name_list = []
            for line in myDataList:
                entry = line.split((","))
                name_list.append(entry[0])
            if ((recognised_student_roll not in name_list) and (recognised_student_name not in name_list) and (
                    recognised_student_dep not in name_list)):
                now = datetime.now()
                d1 = now.strftime("%d/%m/%Y")
                dtString = now.strftime("%H:%M:%S")
                attendence.writelines(
                    f"\n{recognised_student_roll},{recognised_student_name},{recognised_student_dep},{dtString},{d1},Present")
